---
title: "API de CT-e: consultar e cancelar Conhecimentos de Transporte"
sidebarTitle: "CT-e"
description: "Endpoints para consultar CT-es emitidos, buscar por chave de NF-e vinculada, criar minuta a partir de um CT-e e cancelar CT-e."
---

Estes endpoints operam sobre CT-es (Conhecimento de Transporte Eletrônico) já existentes no sistema, na base `/dfe`. Para **emitir** um novo CT-e a partir de dados de uma minuta, veja a página [Entregas e minutas](/api-reference/entregas). Todos os endpoints exigem autenticação via Bearer Token no cabeçalho `Authorization`.

---

## Consultar CT-e pela chave da NF-e

Efetua consulta dos CT-es emitidos pela chave da nota fiscal

**`GET /dfe/cte/nota`**

### Parâmetros de query

<ParamField query="chave_nfe" type="string" required>
  
</ParamField>

### Exemplo de requisição

<CodeGroup>

```bash cURL
curl --request GET \
  --url "https://{cliente}.brudam.com.br/api/v1/dfe/cte/nota?chave_nfe=valor" \
  --header "Authorization: Bearer {seu_token}"
```

</CodeGroup>

### Respostas

**`200`** — Encontrado com sucesso!

```json
{
  "message": "OK",
  "status": 1,
  "data": {
    "xml": ""
  }
}
```

**`400`** — Se os dados enviados estiverem incorretos

```json
{
  "message": "Erro nos dados enviados",
  "status": 0,
  "data": [
    "O campo Senha deve conter um valor de acordo com a E.R. ^[0-9A-Fa-f]{64}.",
    "O campo Senha deve ter exatamente 64 caracteres"
  ]
}
```

**`401`** — Se o usuário não possui permissão para acessar a URL informada ou o TOKEN não for informado.

```json
{
  "message": "Acesso não autorizado",
  "status": 0
}
```

**`403`** — Se os dados de usuário estão incorretos.

```json
{
  "message": "Acesso negado!",
  "status": 0
}
```

**`404`** — Se a URL informada não for encontrada

```json
{
  "message": "Nenhum resultado encontrado!",
  "status": 0
}
```

**`405`** — Se a URL chamada não suporta o método utilizado

```json
{
  "message": "Método não permitido!",
  "status": 0
}
```

---

## Consultar CT-es emitidos

Efetua consulta dos CT-es emitidos

**`GET /dfe/cte`**

### Parâmetros de query

<ParamField query="chave" type="string" required>
  
</ParamField>

### Exemplo de requisição

<CodeGroup>

```bash cURL
curl --request GET \
  --url "https://{cliente}.brudam.com.br/api/v1/dfe/cte?chave=valor" \
  --header "Authorization: Bearer {seu_token}"
```

</CodeGroup>

### Respostas

**`200`** — Encontrado com sucesso!

```json
{
  "message": "OK",
  "status": 1,
  "data": {
    "xml": ""
  }
}
```

**`400`** — Se os dados enviados estiverem incorretos

```json
{
  "message": "Erro nos dados enviados",
  "status": 0,
  "data": [
    "O campo Senha deve conter um valor de acordo com a E.R. ^[0-9A-Fa-f]{64}.",
    "O campo Senha deve ter exatamente 64 caracteres"
  ]
}
```

**`401`** — Se o usuário não possui permissão para acessar a URL informada ou o TOKEN não for informado.

```json
{
  "message": "Acesso não autorizado",
  "status": 0
}
```

**`403`** — Se os dados de usuário estão incorretos.

```json
{
  "message": "Acesso negado!",
  "status": 0
}
```

**`404`** — Se a URL informada não for encontrada

```json
{
  "message": "Nenhum resultado encontrado!",
  "status": 0
}
```

**`405`** — Se a URL chamada não suporta o método utilizado

```json
{
  "message": "Método não permitido!",
  "status": 0
}
```

---

## Criar minuta a partir de um CT-e

Insere no sistema uma nova minuta com base no CT-e enviado. Somente são permitidos **CT-es autorizados** nos últimos **30 dias**.

**`POST /dfe/cte`**

### Corpo da requisição

<ParamField body="documentos" type="array of objects">
  

  <Expandable title="Cada item de documentos">
    <ParamField body="xMemo" type="string">
      Observações
    </ParamField>
    <ParamField body="xml" type="string">
      Arquivo XML do CT-e codificado em Base64.
    </ParamField>
    <ParamField body="dPrev" type="string">
      Data da Previsão de Entrega. Formato Y-m-d
    </ParamField>
    <ParamField body="cServ" type="integer">
      Código do Serviço
    </ParamField>
    <ParamField body="rSeg" type="integer">
      Responsável pelo Seguro

      Valores aceitos: `0`, `1`, `2`, `3`, `4`, `5`.
    </ParamField>
    <ParamField body="cSeg" type="integer">
      CNPJ da Seguradora
    </ParamField>
    <ParamField body="averbacao" type="string">
      Número de averbação
    </ParamField>
    <ParamField body="perecivel" type="boolean">
      Carga é perecível?
    </ParamField>
    <ParamField body="agendamento" type="object">
  

      <Expandable title="Propriedades de agendamento">
        <ParamField body="data" type="string">
          Data do agendamento. Formato Y-m-d
        </ParamField>
        <ParamField body="hIni" type="string">
          Hora Inicial do agendamento. Formato H:i
        </ParamField>
        <ParamField body="HFim" type="string">
          Hora Final do agendamento. Formato H:i
        </ParamField>
      </Expandable>
    </ParamField>
    <ParamField body="carga" type="object">
  

      <Expandable title="Propriedades de carga">
        <ParamField body="pBru" type="number">
          Peso bruto total da carga.
        </ParamField>
        <ParamField body="pCub" type="number">
          Peso cubado total da carga.
        </ParamField>
        <ParamField body="qVol" type="number">
          Quantidade de volumes total da carga.
        </ParamField>
        <ParamField body="vTot" type="number">
          Valor total da carga.
        </ParamField>
      </Expandable>
    </ParamField>
    <ParamField body="documentos" type="array of objects">
  

      <Expandable title="Cada item de documentos">
        <ParamField body="chave" type="string">
          Chave de acesso do documento
        </ParamField>
        <ParamField body="nPed" type="string">
          Número do pedido
        </ParamField>
        <ParamField body="nDoc" type="integer">
          Número do documento
        </ParamField>
        <ParamField body="serie" type="integer">
          Série do documento
        </ParamField>
        <ParamField body="dEmi" type="string">
          Data de emissão do documento. Formato Y-m-d
        </ParamField>
        <ParamField body="vBC" type="number">
          Base de cálculo ICMS
        </ParamField>
        <ParamField body="vICMS" type="number">
          Valor do ICMS
        </ParamField>
        <ParamField body="vBCST" type="number">
          Base de cálculo ICMS ST
        </ParamField>
        <ParamField body="vST" type="number">
          Valor do ICMS ST
        </ParamField>
        <ParamField body="vProd" type="number">
          Valor dos produtos
        </ParamField>
        <ParamField body="vNF" type="number">
          Valor do documento
        </ParamField>
        <ParamField body="nCFOP" type="integer">
          CFOP do documento
        </ParamField>
        <ParamField body="pBru" type="number">
          Peso bruto
        </ParamField>
        <ParamField body="qVol" type="number">
          Quantidade de volumes
        </ParamField>
        <ParamField body="PIN" type="string">
          PIN
        </ParamField>
        <ParamField body="xEsp" type="string">
          Espécie da mercadoria
        </ParamField>
        <ParamField body="xNat" type="string">
          Natureza da mercadoria
        </ParamField>
      </Expandable>
    </ParamField>
    <ParamField body="cubagem" type="array of objects">
  

      <Expandable title="Cada item de cubagem">
        <ParamField body="dCom" type="number">
          Comprimento em metros
        </ParamField>
        <ParamField body="dLar" type="number">
          Largura em metros
        </ParamField>
        <ParamField body="dAlt" type="number">
          Altura em metros
        </ParamField>
        <ParamField body="pBru" type="number">
          Peso Bruto
        </ParamField>
        <ParamField body="pCub" type="number">
          Peso Cubado
        </ParamField>
      </Expandable>
    </ParamField>
  </Expandable>
</ParamField>

### Exemplo de requisição

<CodeGroup>

```bash cURL
curl --request POST \
  --url "https://{cliente}.brudam.com.br/api/v1/dfe/cte" \
  --header "Authorization: Bearer {seu_token}" \
  --header "Content-Type: application/json" \
  --data '{
  "documentos": [
    {
      "xMemo": "string",
      "xml": "string",
      "dPrev": "string",
      "cServ": 0
    }
  ]
}'
```

</CodeGroup>

### Respostas

**`201`** — Inserido com sucesso!

```json
{
  "message": "Inserido com sucesso!",
  "status": 1,
  "data": [
    {
      "chave": "35190607575651001554570130001077981519594692",
      "status": 1,
      "mensagem": "Dados registrado com sucesso!"
    }
  ]
}
```

**`400`** — Se os dados enviados estiverem incorretos

```json
{
  "message": "Erro nos dados enviados",
  "status": 0,
  "data": [
    "O campo Senha deve conter um valor de acordo com a E.R. ^[0-9A-Fa-f]{64}.",
    "O campo Senha deve ter exatamente 64 caracteres"
  ]
}
```

**`401`** — Se o usuário não possui permissão para acessar a URL informada ou o TOKEN não for informado.

```json
{
  "message": "Acesso não autorizado",
  "status": 0
}
```

**`403`** — Se os dados de usuário estão incorretos.

```json
{
  "message": "Acesso negado!",
  "status": 0
}
```

**`404`** — Se a URL informada não for encontrada

```json
{
  "message": "Nenhum resultado encontrado!",
  "status": 0
}
```

**`405`** — Se a URL chamada não suporta o método utilizado

```json
{
  "message": "Método não permitido!",
  "status": 0
}
```

---

## Cancelar CT-e

Efetua consulta dos CT-es emitidos

**`GET /dfe/cancela/cte/{chaveCte}`**

### Parâmetros de caminho

<ParamField path="chaveCte" type="string" required>
  
</ParamField>

### Exemplo de requisição

<CodeGroup>

```bash cURL
curl --request GET \
  --url "https://{cliente}.brudam.com.br/api/v1/dfe/cancela/cte/{chaveCte}" \
  --header "Authorization: Bearer {seu_token}"
```

</CodeGroup>

### Respostas

**`200`** — Cancelado com sucesso!

```json
{
  "message": "OK",
  "status": 1,
  "data": {
    "message": "A minuta 383845 foi cancelada com sucesso."
  }
}
```

**`400`** — Se os dados enviados estiverem incorretos

```json
{
  "message": "Erro nos dados enviados",
  "status": 0,
  "data": [
    "O campo Senha deve conter um valor de acordo com a E.R. ^[0-9A-Fa-f]{64}.",
    "O campo Senha deve ter exatamente 64 caracteres"
  ]
}
```

**`401`** — Se o usuário não possui permissão para acessar a URL informada ou o TOKEN não for informado.

```json
{
  "message": "Acesso não autorizado",
  "status": 0
}
```

**`403`** — Se os dados de usuário estão incorretos.

```json
{
  "message": "Acesso negado!",
  "status": 0
}
```

**`404`** — Se a URL informada não for encontrada

```json
{
  "message": "Nenhum resultado encontrado!",
  "status": 0
}
```

**`405`** — Se a URL chamada não suporta o método utilizado

```json
{
  "message": "Método não permitido!",
  "status": 0
}
```

---
